...
import com.github.processor.utils.ParametersUtils;
...
public class PutMethodFactory implements RESTMethodsFactory {

    @Override
    public MethodSpec method(Element method, Arthas arthas) {
        AnnotationRESTMethodsEmulator annotation = new AnnotationRESTMethodsEmulator(method, AnnotationRESTMethodsEmulator.PUT_MAPPING);
        if (annotation.isAnnotationExist()) {
            ...
            if (methodReturnType instanceof DeclaredType) {
                ...
                List<ParameterSpec> convertedParameters = ParametersUtils.convertedParameters(parameters);
